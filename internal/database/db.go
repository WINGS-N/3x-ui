...
func runSeeders(isUsersEmpty bool) error {
	...
	if empty && isUsersEmpty {
		seeders := []string{"UserPasswordHash", "ClientsTable", "InboundClientsArrayFix", "InboundClientTgIdFix", "InboundClientSubIdFix", "FreedomFinalRulesReverseFix", "ApiTokensHash", "LegacyProxySettingsCleanup", "WireguardPeersToClients", "MtprotoSecretsToClients", "NodeInboundsAdopted", "VKTurnAcceptClientKeysDefaultOn"}
	...
	// Self-gated on the "WireguardPeersToClients" row. Destructive by design: it
	// rewrites settings.peers into settings.clients and drops the inline list, so
	// everything that touches WireGuard peers (including the vk-turn-proxy peer
	// mirroring) must read the clients model.
