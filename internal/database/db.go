...
func runSeeders(isUsersEmpty bool) error {
	...
	// WINGS fork: upstream's WireguardPeersToClients migration is intentionally
	// NOT run. It rewrites settings.peers into settings.clients, but this fork
	// keeps WireGuard peers inline (settings.peers); the vk-turn-proxy peer
	// mirroring and allowedIP reconciliation read and write that inline list
	// directly, so converting to the clients model would break them.
	return nil
}
