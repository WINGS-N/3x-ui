...
func (s *InboundService) AddInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		// vk-turn-proxy inbounds are not pushed to the xray runtime (they are
		// reconciled separately by applyVKTurnProxyInboundOnSave), so gate the
		// node/runtime sync to xray-managed protocols only.
...
func (s *InboundService) UpdateInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	txErr := runSerializedTx(func(tx *gorm.DB) error {
		...
		if oldInbound.NodeID == nil {
			// vk-turn-proxy inbounds are not pushed to the xray runtime (they are
			// reconciled separately by applyVKTurnProxyInboundOnSave), so gate the
			// runtime sync to xray-managed protocols only.
			if isXrayManagedProtocol(oldInbound.Protocol) {
